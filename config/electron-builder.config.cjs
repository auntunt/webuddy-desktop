...
const devChannelBuildVersion = isHourlyChannel
  ...
  : isDailyChannel
    ...
    : isAdhocChannel
      ...
      : undefined
// Why no dev-channel repo split: Webuddy ships internally and has no published
// release feed to strand users on, so the hourly/daily/adhoc channels all build
// the same local artifact identity.
const appId = 'cn.cloudwave.webuddy'
...
// Why extraResources rather than app.asar: the app spawns this collector as a
// plain Node program (ELECTRON_RUN_AS_NODE), so it has to exist as real files
// on disk. Keeping it unbundled also means the same source is what the server
// tooling and any manual run use.
const sessionCollectorResources = {
  from: 'tools/webuddy-agent',
  to: 'webuddy-agent',
...
module.exports = {
  appId,
  productName: 'Webuddy',
  protocols: [{ name: 'Webuddy', schemes: ['webuddy'] }],
  ...
  files: [
    ...
    '!Casks{,/**/*}',
    // Why: cloud/ is the relay *server* workspace (its own pnpm project, deployed
    // separately) and tools/ is the local collector CLI. Neither is a runtime
    // dependency of the desktop app, and shipping them leaked the relay's
    // production endpoints and its test files into app.asar.
    '!cloud{,/**/*}',
    '!tools{,/**/*}',
    // Why: dev-only tooling and build config. LICENSE stays packed — the MIT
    // license has to travel with the distributed app.
    '!{.oxfmtrc.json,.oxlintrc.json,components.json,orca.yaml,pnpm-workspace.yaml,vite.web.config.ts}',
  ...
  afterPack: async (context) => {
    ...
    if (context.electronPlatformName === 'darwin') {
      await signMacComputerUseHelper(join(resourcesDir, 'Webuddy Computer Use.app'), context.packager)
  ...
  nsis: {
    artifactName: 'webuddy-windows-setup.${ext}',
  ...
  mac: {
    ...
    extendInfo: {
      NSAppleEventsUsageDescription:
        'Webuddy allows terminal-launched developer tools to automate local apps when you request it.',
      NSBluetoothAlwaysUsageDescription:
        'Webuddy allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSBluetoothPeripheralUsageDescription:
        'Webuddy allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSCameraUsageDescription: "Application requests access to the device's camera.",
      NSLocationUsageDescription:
        'Webuddy allows terminal-launched developer tools to access location when you request it.',
      NSLocalNetworkUsageDescription:
        'Webuddy allows terminal-launched developer tools to discover and connect to local development servers when you request it.',
      NSMicrophoneUsageDescription: "Application requests access to the device's microphone.",
      NSAudioCaptureUsageDescription:
        'Webuddy allows terminal-launched developer tools to capture desktop audio when you request it.',
    ...
    extraResources: [
      ...
      {
        from: 'native/computer-use-macos/.build/release/Webuddy Computer Use.app',
        to: 'Webuddy Computer Use.app'
      },
      featureWallResources
    ],
  ...
  dmg: {
    artifactName: 'webuddy-macos-${arch}.${ext}'
  },
  linux: {
    ...
    desktop: {
      entry: {
        // Why: Electron reports WM_CLASS for the visible Linux window; GNOME
        // docks need an exact match to group it with webuddy.desktop.
        StartupWMClass: 'webuddy'
      }
    },
    extraResources: [
      ...commonExtraResources,
    ...
    maintainer: 'cloudwave',
    category: 'Utility'
  },
  appImage: {
    artifactName: isLinuxArm64Release ? 'webuddy-linux-arm64.${ext}' : 'webuddy-linux.${ext}'
  },
  deb: {
    packageName: 'webuddy',
    artifactName: 'webuddy_${version}_${arch}.${ext}',
  ...
  rpm: {
    packageName: 'webuddy',
    artifactName: 'webuddy-${version}.${arch}.${ext}',
  ...
  npmRebuild: true
  // Why: Webuddy ships internally and must never poll the upstream Orca release
  // feed. Omitting `publish` means electron-builder writes no app-update.yml, so
  // electron-updater has no feed to reach and checkForUpdates() fails closed.
}
...
async function signMacComputerUseHelper(helperAppPath, packager) {
  if (!existsSync(helperAppPath)) {
    if (isMacRelease) {
      throw new Error(`Missing Webuddy Computer Use helper app at ${helperAppPath}`)
    }
    return
  }
  ...
  const identity =
    ...
    (isMacRelease ? null : '-')
  if (!identity) {
    throw new Error('Missing signing identity for Webuddy Computer Use helper app')
  }
