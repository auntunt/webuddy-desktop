...
- 退出登录时服务端吊销该用户**所有** `collector:%` token，以及该用户**所有** desktop refresh token（不限发起退出的设备）：其他设备的 access token 在 ≤1h 内过期，刷新收到 401 后回到登录页并清空本机采集器凭证——即"任一设备退出，全部设备退出"，非"桌面会话仍有效、仅采集器重签"。
