...
async function createMarketplaceFixture(): Promise<MarketplaceFixture> {
  ...
  await copyLaunchPlugin(
    repositories,
    'orca-portuguese',
    'cloudwave.webuddy-portuguese',
    gitEnvironment
  )
  await copyLaunchPlugin(
    repositories,
    'orca-multipass-recipes',
    'cloudwave.webuddy-multipass-recipes',
    gitEnvironment
  )
  await copyLaunchPlugin(
    repositories,
    'orca-navigation-shortcuts',
    'cloudwave.webuddy-navigation-shortcuts',
    gitEnvironment
  )
  ...
  await writeFile(
    join(marketplaceRepository, 'webuddy-marketplace.json'),
    `${JSON.stringify(
      {
        name: 'Orca Plugins',
        owner: 'stablyai',
        plugins: [
          ['cloudwave.webuddy-portuguese', 'orca-portuguese', 'languages'],
          ['cloudwave.webuddy-multipass-recipes', 'orca-multipass-recipes', 'vm-recipes'],
          ['cloudwave.webuddy-navigation-shortcuts', 'orca-navigation-shortcuts', 'keybindings']
...
async function applyInstalledLanguage(page: Page): Promise<void> {
  const languageId = 'plugin:cloudwave.webuddy-portuguese/pt-BR'
  await page.evaluate(() => {
    const state = window.__store?.getState()
    if (!state) {
      throw new Error('store unavailable')
  ...
  await page
    .getByRole('option', { name: 'pt-BR — cloudwave.webuddy-portuguese', exact: true })
    .click()
  await expect
  ...
  await expect
    ...
    .toMatchObject({
      ...
      listings: expect.arrayContaining([
        expect.objectContaining({ pluginKey: 'cloudwave.webuddy-portuguese', official: true }),
        expect.objectContaining({
          pluginKey: 'cloudwave.webuddy-multipass-recipes',
          official: true
        }),
        expect.objectContaining({
          pluginKey: 'cloudwave.webuddy-navigation-shortcuts',
          official: true
        })
      ])
    })
