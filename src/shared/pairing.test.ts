...
describe('pairing offer', () => {
  ...
  it('rejects orca URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`webuddy://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`webuddy://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`webuddy://pairing?code=${code}`)).toThrow(
      'Invalid pairing URL'
    )
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('webuddy://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes legacy hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`webuddy://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`webuddy://pair#${base64}`)).toThrow()
  })
...
describe('parsePairingCode', () => {
  ...
  it('parses a full webuddy://pair# URL', () => {
    const url = encodePairingOffer(offer)
