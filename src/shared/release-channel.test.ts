...
describe('release channel', () => {
  ...
  it('builds release-notes links against the repo that published the version', () => {
    expect(getReleaseNotesUrlForVersion('1.4.160-hourly.202607281400')).toBe(
      'https://github.com/cloudwave/webuddy-hourly/releases/tag/v1.4.160-hourly.202607281400'
    )
  ...
  it('sorts consecutive hourly builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/cloudwave/webuddy-hourly/releases/tag/v${version}`,
      installerUrl: null
    })
  ...
  it('sorts consecutive adhoc builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/cloudwave/webuddy-adhoc/releases/tag/v${version}`,
      installerUrl: null
    })
    const sorted = sortReleaseBuildsNewestFirst([
