...
const PRODUCTION_HOSTS = new Set(['app.cloudwaveai.cn', 'share.cloudwaveai.cn'])
...
export function skillShareIdFromArguments(argv: readonly string[]): string | null {
  for (const value of argv) {
    ...
    if (id && (value.includes('/skills/share/') || value.startsWith('webuddy:'))) {
      return id
    }
  }
  return null
