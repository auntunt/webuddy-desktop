...
export function verifyRemotePairingRuntimeStatus(
  ...
  if (status.deviceScope === 'mobile') {
    return {
      ...
      message: 'This link grants mobile-only access. Generate a link for another Webuddy client.'
    }
  }
  const versionFields = [
    ...
    return {
      ...
      message:
        compatibility.reason === 'client-too-old'
          ? 'Update this Webuddy client before adding the remote host.'
          : 'Update Webuddy on the remote host before adding it.'
    }
  }
