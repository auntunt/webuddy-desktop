...
const PRODUCTION_ARTIFACTS_API_URL = 'https://share.cloudwaveai.cn'
...
export function resolveArtifactCloudApiUrl(
  ...
  const firstParty = url.hostname === 'cloudwaveai.cn' || url.hostname.endsWith('.cloudwaveai.cn')
  ...
  if (!firstParty && !loopback) {
    throw new Error('Artifact API URLs must use a cloudwaveai.cn or loopback host.')
  }
