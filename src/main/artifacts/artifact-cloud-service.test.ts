...
function createResponse(
  ...
  return new Response(
    JSON.stringify({
      ...
      shareUrl: `https://share.cloudwaveai.cn/a/${slug}`,
...
describe('ArtifactCloudService record authorization', () => {
  ...
  it('creates once and updates on repeated publish', async () => {
    ...
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'created', item: { shareUrl: 'https://share.cloudwaveai.cn/a/artifact-a' } }
    })
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'updated', item: { shareUrl: 'https://share.cloudwaveai.cn/a/artifact-a' } }
    })
  ...
  it('resolves a persisted public link only for its source and cloud scope', async () => {
    ...
    await expect(
      ...
      status: 'ok',
      value: { shareUrl: 'https://share.cloudwaveai.cn/a/artifact-a' }
    })
    await expect(
  ...
  it('serializes manual publish with CLI share for the same source', async () => {
    ...
    await expect(Promise.all([publish, share])).resolves.toMatchObject([
      { status: 'ok', value: { change: 'created' } },
      { status: 'ok', value: { shareUrl: 'https://share.cloudwaveai.cn/a/artifact-b' } }
    ])
    expect(fetchMock).toHaveBeenCalledTimes(2)
  })
  ...
  it('recreates an artifact when its stored public link was deleted elsewhere', async () => {
    ...
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'created', item: { shareUrl: 'https://share.cloudwaveai.cn/a/artifact-b' } }
    })
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'updated', item: { shareUrl: 'https://share.cloudwaveai.cn/a/artifact-b' } }
    })
