...
const ORCA_DOMAIN = 'cn.cloudwave.webuddy'
...
describe('ensureMacPressAndHoldDefault', () => {
  ...
  describe('domains we do not own', () => {
    ...
    it('accepts Orca and its channel-scoped bundles, and nothing else', () => {
      expect(isOrcaPreferencesDomain('cn.cloudwave.webuddy')).toBe(true)
      expect(isOrcaPreferencesDomain('cn.cloudwave.webuddy.dev')).toBe(true)
      ...
      expect(isOrcaPreferencesDomain('cn.cloudwave.webuddyfake')).toBe(false)
    })
  })
  ...
  it('reads CFBundleIdentifier from the plist beside the executable', () => {
    const exe = bundleWithPlist(
      '<plist><dict>\n<key>CFBundleName</key>\n<string>Orca</string>\n' +
        '<key>CFBundleIdentifier</key>\n\t<string>cn.cloudwave.webuddy</string>\n</dict></plist>'
    )

    expect(readBundleIdentifierFromExecutablePath(exe)).toBe('cn.cloudwave.webuddy')
  })
