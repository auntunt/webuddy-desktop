...
const ATOM_FEED_URL = 'https://github.com/cloudwave/webuddy/releases.atom'
const RELEASES_DOWNLOAD_BASE = 'https://github.com/cloudwave/webuddy/releases/download'
...
const TAG_HREF_RE = /href="https:\/\/github\.com\/cloudwave\/webuddy\/releases\/tag\/([^"]+)"/g
...
async function getReleaseAssetReadiness(tag: string, assetName: string): Promise<ReleaseReadiness> {
  ...
  const isGitHubReleaseAsset =
    ...
    (isRelativeAsset ||
      /^https:\/\/github\.com\/cloudwave\/webuddy\/releases\/download\//i.test(assetName))
