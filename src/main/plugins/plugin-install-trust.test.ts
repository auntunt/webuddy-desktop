...
describe('plugin install trust', () => {
  ...
  it('rejects locally installed reserved identities before publication', async () => {
    ...
    await expect(
      ...
      ok: false,
      error:
        'reserved plugin identity cloudwave.webuddy-skills cannot be installed from a local path'
    })
  ...
  it('allows the app-bundled path only for the complete official identity', async () => {
    ...
    const result = await installBundledPlugin({
      ...
      sourcePath,
      hostVersion: '1.4.0',
      expectedPluginKey: 'cloudwave.webuddy-skills'
    })

    expect(result).toMatchObject({ ok: true, pluginKey: 'cloudwave.webuddy-skills' })
    const lock = await readPluginLockfile(pluginsDir)
    expect(lock.plugins['cloudwave.webuddy-skills']?.source).toEqual({
      kind: 'bundled',
      bundleId: 'cloudwave.webuddy-skills'
    })
  })
