...
describe('parseTccPromptEvent', () => {
  ...
  it('separates the accessing binary from the responsible app', () => {
    ...
    expect(event?.responsibleIdentifier).toBe('cn.cloudwave.webuddy')
...
describe('isOrcaAttributedPrompt', () => {
  it('accepts the app and detached terminal helper across Orca build identities', () => {
    for (const id of [
      'cn.cloudwave.webuddy',
      'cn.cloudwave.webuddy.helper',
      'cn.cloudwave.webuddy.dev',
      'cn.cloudwave.webuddy.dev.helper',
      'cn.cloudwave.webuddy.local',
      'cn.cloudwave.webuddy.local.helper'
    ]) {
      expect(
        isOrcaAttributedPrompt({
  ...
  it('rejects unrelated services even when Orca is responsible', () => {
    expect(
      isOrcaAttributedPrompt({
        ...
        responsibleIdentifier: 'cn.cloudwave.webuddy'
      })
    ).toBe(false)
  })
})
