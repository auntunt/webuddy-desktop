export function getPiAgentStatusWslCurlSourceLines(): string[] {
  return [
    ...
    "  if (!curlPath || !windowsPort || !windowsToken) throw new Error('Webuddy WSL hook bridge unavailable')",
    ...
    "      reject(new Error('Webuddy WSL hook delivery timed out'))",
    '    }, 11000)',
    ...
    '      clearTimeout(timer)',
    "      if (code === 0) resolve(); else reject(new Error('Webuddy WSL hook exit ' + code))",
    '    })',
