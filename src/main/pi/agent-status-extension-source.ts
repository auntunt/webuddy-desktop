...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '// Webuddy receiver from building an unbounded queue of obsolete snapshots.',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('Webuddy hook delivery timed out'))",
    ...
    '      timeoutPromise,',
    '    ])',
    "    if (!response.ok) throw new Error('Webuddy hook HTTP ' + response.status)",
    '  } catch (error) {',
    '    // Why: status reporting must never fail the pi run just because Webuddy',
    '    // is unavailable or the loopback request failed (e.g. Webuddy restart).',
