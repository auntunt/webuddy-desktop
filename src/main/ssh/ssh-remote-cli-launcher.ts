...
internal static class OrcaRemoteCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            {
                Console.Error.WriteLine("Webuddy SSH CLI bridge cannot find Node.js at \"{0}\"", nodePath);
                return 1;
            }
            if (!File.Exists(relayPath))
            {
                Console.Error.WriteLine("Webuddy SSH CLI bridge cannot find the relay at \"{0}\"", relayPath);
                return 1;
            }
        ...
        {
            Console.Error.WriteLine("Unable to start the Webuddy SSH CLI bridge: {0}", error.Message);
            return 1;
        }
    }
...
function createWindowsLauncherCompileCommand(
  ...
  return powerShellCommand(
    [
      ...
      "if (-not $compiler) { Write-Error 'Unable to find the .NET Framework C# compiler required for the Webuddy SSH CLI launcher.'; exit 1 }",
    ...
    files: [
      {
        ...
        contents: [
          ...
          '  echo "Webuddy SSH CLI bridge cannot find the relay socket: $ORCA_RELAY_SOCKET_PATH" >&2',
          '  exit 1',
          'fi',
