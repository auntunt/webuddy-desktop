...
function latchedMessage(record: AgentSessionRecord): string {
  ...
  if (record.lease.settlementRetryRequired) {
    return 'The provider exited, but Webuddy has not finished settling the terminal chat state. Reopen this chat to retry the settlement.'
  }
  if (record.lease.claimStatus === 'conflicted') {
    return owner
      ...
      : 'Two runtimes claimed this session and the record names no process to check. Quit any other Webuddy or agent process using this workspace, then reopen this chat.'
  }
  return owner
    ...
    : 'Webuddy cannot tell whether an agent started for this session before the app stopped, so it will not start a second one on the same conversation. Quit any leftover agent process for this workspace and reopen this chat.'
}
...
export function structuredAgentSessionRefusalMessage(
  ...
  if (code === 'execution_owner_reconciling') {
    return 'Webuddy is still working out who owns this session on this machine. Reopen the chat in a moment.'
  }
  return null
}
