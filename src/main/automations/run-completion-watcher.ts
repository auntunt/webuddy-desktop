...
export function describeStrandedAutomationRun(run: AutomationRun): string {
  if (run.status === 'dispatching') {
    return 'Webuddy stopped before this run reported that its agent started.'
  }
  return 'Webuddy lost the terminal for this run before it reported completion.'
}
...
function describeObservationError(error: unknown): string {
  ...
  return 'Webuddy stopped watching this run before it reported completion.'
}
