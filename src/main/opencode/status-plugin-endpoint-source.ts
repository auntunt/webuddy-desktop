export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// OpenCode's Node process (not Webuddy's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Webuddy restart',
    ...
    '      // writeEndpointFile on the Webuddy side) allocates a fresh inode on',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Webuddy restart. The',
    '  // file is rewritten on every Webuddy start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Webuddy',
