...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'cn.cloudwave.webuddy.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Webuddy Computer Use.app was not found')
  }
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Webuddy Computer Use, then retry get-app-state.`
}
