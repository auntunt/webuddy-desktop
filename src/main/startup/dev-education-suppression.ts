...
/**
 * Why this no longer keys off `app.isPackaged`: upstream ships first-run
 * education (onboarding wizard, feature tips, contextual tours) to teach new
 * Orca users, but this is internal company software — every user is a developer
 * who should land straight in the editor. The two env escape hatches remain so
 * the first-run surfaces stay testable.
 */
export function shouldSuppressFirstRunEducation(
  args: {
    env?: NodeJS.ProcessEnv
  } = {}
): boolean {
  const env = args.env ?? process.env
  return env.ORCA_E2E_USER_DATA_DIR === undefined && env[DEV_SHOW_FIRST_RUN_EDUCATION_ENV] !== '1'
}
