...
export async function initializeReadyRuntimeServices(): Promise<void> {
  ...
  // Why no StarNagService: upstream prompts users to star the public Orca repo.
  // This is an internal fork — there is no public listing to promote, so the
  // service is never started and no toast host is ever mounted.
