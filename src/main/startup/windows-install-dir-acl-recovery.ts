...
const CAUSE =
  "Windows permissions on Webuddy's install folder are blocking its own sandboxed processes from reading the files it shipped with."
...
export function describeInstallDirAclPoison(): InstallDirAclPoisonDiagnosis | null {
  ...
  const status =
    poison.stage === 'pending'
      ? 'Webuddy is repairing the permissions now.'
      : 'Webuddy could not repair them, which usually means the folder needs an administrator.'
  return {
