...
describe('SkillShareDeepLinkState', () => {
  it('queues a startup share until the renderer consumes it once', () => {
    ...
    expect(state.capture(['orca', 'https://app.cloudwaveai.cn/skills/share/share_startup'])).toBe(
      true
    )
  ...
  it('publishes a later second-instance intent and keeps it for renderer recovery', () => {
    ...
    state.capture(['orca', 'https://app.cloudwaveai.cn/skills/share/share_first'])
    expect(state.capture(['orca', 'webuddy://skills/share/share_second'], publish)).toBe(true)
  ...
  it('ignores untrusted URLs without replacing a pending intent', () => {
    const state = new SkillShareDeepLinkState()
    state.capture(['orca', 'https://app.cloudwaveai.cn/skills/share/share_safe'])
