...
describe('dev-instance-identity', () => {
  it('keeps packaged identity stable', () => {
    expect(getDevInstanceIdentity(false, {})).toMatchObject({
      ...
      devLabel: null,
      dockBadgeLabel: null,
      appUserModelId: 'cn.cloudwave.webuddy'
    })
  })
  ...
  it('pins a stable dev appName across branches so the safeStorage key does not churn', () => {
    ...
    expect(a.appName).toBe('Webuddy Dev')
    expect(b.appName).toBe('Webuddy Dev')
    expect(a.appName).not.toBe('Webuddy')
  })
  ...
  it('derives a readable dev label from worktree and branch env', () => {
    ...
    expect(identity.name).toBe('Webuddy: nwparker/dev-indicator')
    expect(identity.dockBadgeLabel).toBeNull()
    expect(identity.appUserModelId).toMatch(/^cn\.cloudwave\.webuddy\.dev\.[a-f0-9]{10}$/)
  })
  ...
  it('includes the branch when it differs from the worktree basename', () => {
    ...
    expect(identity.name).toBe('Webuddy: feature/billing-shell')
    expect(identity.dockBadgeLabel).toBeNull()
  })
  ...
  it('allows an explicit label override', () => {
    ...
    expect(identity.name).toBe('Webuddy: feature/other')
    expect(identity.dockBadgeLabel).toBeNull()
  })
})
