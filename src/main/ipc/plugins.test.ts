...
describe('plugin removal authority', () => {
  ...
  it('refuses bundled installs because startup would restore them', () => {
    const service = {
      getDiscovered: () => [{ pluginKey: 'cloudwave.webuddy-theme', isDev: false }]
    } as unknown as PluginService
    const lock = {
      version: 1,
      plugins: {
        'cloudwave.webuddy-theme': {
          pluginKey: 'cloudwave.webuddy-theme',
          version: '1.0.0',
          source: { kind: 'bundled', bundleId: 'cloudwave.webuddy-theme' },
          resolvedCommit: null,
    ...
    expect(canRemoveInstalledPlugin(service, 'cloudwave.webuddy-theme', lock)).toBe(false)
  })
})
