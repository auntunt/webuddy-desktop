...
function precommitRefusal(error: unknown): AgentSessionWireRefusal {
  ...
  if (code) {
    return { code, message: 'Webuddy cannot open a structured agent chat for this workspace.' }
  }
  ...
  return {
    ...
    message: `Webuddy could not prepare a structured agent chat for this workspace: ${message}`
  }
}
