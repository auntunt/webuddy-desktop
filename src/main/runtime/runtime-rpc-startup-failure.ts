...
> = {
  permission_denied: {
    ...
    fallback:
      "Webuddy couldn't write its runtime file. Check permissions on Webuddy's data folder, then restart."
  },
  storage_unavailable: {
    key: 'runtimeRpc.startupFailure.guidance.storageUnavailable',
    fallback: 'Your disk may be full or read-only. Free up space, then restart Webuddy.'
  },
  invalid_path: {
    key: 'runtimeRpc.startupFailure.guidance.invalidPath',
    fallback:
      "Webuddy's data folder may be missing, moved, or at a path that is too long. Restore it or use a shorter path, then restart Webuddy."
  },
  address_in_use: {
    key: 'runtimeRpc.startupFailure.guidance.addressInUse',
    fallback: 'Another process may be holding the port. Restart Webuddy to try again.'
  },
  unknown: {
    key: 'runtimeRpc.startupFailure.guidance.unknown',
    fallback: 'Restart Webuddy to try again.'
  }
}
...
function createRuntimeRpcStartupFailureDialogOptions(error: unknown): MessageBoxOptions {
  ...
  return {
    ...
    cancelId: 0,
    noLink: true,
    title: translateMain('runtimeRpc.startupFailure.title', 'Webuddy CLI unavailable'),
    message: translateMain(
      'runtimeRpc.startupFailure.message',
      "Webuddy couldn't start its local command transport."
    ),
    detail: translateMain(
      'runtimeRpc.startupFailure.detail',
      'Webuddy will continue to work, but commands such as orca status, orca terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}',
