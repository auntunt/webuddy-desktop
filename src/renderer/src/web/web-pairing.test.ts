...
describe('web pairing input', () => {
  ...
  it('parses query-form pairing URLs', () => {
    expect(parseWebPairingInput(`webuddy://pair?code=${encodeOffer()}`)).toEqual(offer)
  })

  it('still parses legacy hash-form pairing URLs', () => {
    expect(parseWebPairingInput(`webuddy://pair#${encodeOffer()}`)).toEqual(offer)
  })

  it('preserves optional device scope metadata', () => {
    expect(parseWebPairingInput(`webuddy://pair?code=${encodeOffer({ scope: 'mobile' })}`)).toEqual(
      {
        ...offer,
        scope: 'mobile'
      }
  ...
  it('preserves optional paired device identity', () => {
    expect(
      parseWebPairingInput(
        `webuddy://pair?code=${encodeOffer({ pairedDeviceId: 'paired-device-a' })}`
      )
    ).toEqual({
      ...offer,
  ...
  it('treats invalid device scope metadata as unknown', () => {
    expect(parseWebPairingInput(`webuddy://pair?code=${encodeOffer({ scope: 'admin' })}`)).toEqual(
      offer
    )
  })

  it('rejects orca URLs outside the exact pairing route', () => {
    expect(parseWebPairingInput(`webuddy://pairing?code=${encodeOffer()}`)).toBeNull()
    expect(parseWebPairingInput(`webuddy://pair-extra?code=${encodeOffer()}`)).toBeNull()
  })

  it('auto-saves scoped runtime offers during web startup', () => {
    const input = `webuddy://pair?code=${encodeOffer({ scope: 'runtime' })}`
    expect(
  ...
  it('shows the connect screen for mobile-scope and legacy unknown-scope offers', () => {
    const mobileInput = `webuddy://pair?code=${encodeOffer({ scope: 'mobile' })}`
    const legacyInput = `webuddy://pair?code=${encodeOffer()}`
