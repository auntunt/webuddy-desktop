...
describe('WebRuntimeClient', () => {
  ...
  it('advertises explicit close intent support in encrypted authentication', async () => {
    ...
    client.close()
    await expect(call).rejects.toThrow('Remote Webuddy runtime connection closed.')
  })
  ...
  it('rejects pending connection waiters when the client closes', async () => {
    ...
    try {
      ...
      await expect(callPromise).rejects.toThrow('Remote Webuddy runtime connection closed.')
