...
export function concurrentLookupDetail(input: ChecksPanelReviewStateInput): string | undefined {
  ...
  if (input.reviewLookup === 'positive_unresolved') {
    return translate(
      ...
      'Webuddy also has saved {{reviewLabel}} information that it could not verify.',
      { reviewLabel }
    )
  }
  if (isRateLimitRefresh(refresh)) {
    return translate(
      ...
      'Webuddy also could not check {{reviewLabel}} status because {{provider}} is temporarily limiting requests.',
  ...
  if (refresh?.errorType === 'network') {
    return translate(
      ...
      'Webuddy also could not check {{reviewLabel}} status because this environment could not reach {{provider}}.',
      { reviewLabel, provider: providerName }
    )
  }
  if (refresh?.status === 'error' || isHardRefreshError(refresh)) {
    return translate(
      ...
      'Webuddy also could not confirm whether this branch already has a {{reviewLabel}}.',
      { reviewLabel }
    )
  }
  return undefined
...
export function transientRefreshState(
  ...
  if (refresh?.errorType === 'unknown') {
    return {
      ...
      description: translate(
        ...
        'The lookup failed, so Webuddy could not confirm whether this branch already has a {{reviewLabel}}.',
        { reviewLabel }
      )
    }
  }
  return {
    ...
    description: translate(
      ...
      'Webuddy could not confirm whether this branch already has a {{reviewLabel}}. Retry to check again.',
      { reviewLabel }
    )
  }
}
...
> = {
  ...
  gh_unavailable: {
    ...
    body: {
      ...
      fallback:
        'Webuddy could not run {{provider}} CLI in this environment. Set it up here, then retry.'
    }
  }
}
...
> = {
  disconnected: {
    ...
    body: {
      ...
      fallback:
        "This repository's execution host is disconnected, so Webuddy cannot refresh {{reviewLabel}} status."
    },
    recovery: ['retry']
  },
  bare: {
  ...
  archived: {
    ...
    body: {
      ...
      fallback: 'This repository is archived, so Webuddy is not refreshing {{reviewLabel}} status.'
    },
    recovery: []
  },
  'not-git': {
    ...
    body: {
      ...
      fallback:
        'Webuddy could not treat this folder as a Git repository for {{reviewLabel}} status.'
    },
    recovery: []
  },
  remote: {
    ...
    body: {
      ...
      fallback:
        'Webuddy could not refresh {{reviewLabel}} status for this remote context. Retry after the host is available.'
    },
    recovery: ['retry']
  }
}
