...
describe('ArtifactsPage', () => {
        ...
        artifacts: [
          {
            ...
            shareUrl: 'https://share.cloudwaveai.cn/a/report-123'
          }
        ]
      }
    })
  ...
  it('renders the selected artifact in a right drawer with copy link as the primary action', async () => {
    ...
    await waitFor(() => {
      ...
      expect(preview).toHaveAttribute('src', 'https://share.cloudwaveai.cn/a/report-123?embed=1')
    })

    fireEvent.click(copyButton)
    await waitFor(() =>
      expect(mocks.writeClipboardText).toHaveBeenCalledWith(
        'https://share.cloudwaveai.cn/a/report-123'
      )
    )
    expect(mocks.toastSuccess).toHaveBeenCalledWith('Artifact link copied')

    fireEvent.click(screen.getByRole('button', { name: 'Open in browser' }))
    expect(mocks.openUrl).toHaveBeenCalledWith('https://share.cloudwaveai.cn/a/report-123')
  })
        ...
        artifacts: [
          {
            ...
            shareUrl: 'https://share.cloudwaveai.cn/a/account-a-secret'
          }
        ]
      }
    })
