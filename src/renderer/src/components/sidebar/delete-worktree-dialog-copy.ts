  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Webuddy. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Webuddy. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Webuddy workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Webuddy. The project folder on disk will not be deleted.'
