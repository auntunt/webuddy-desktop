...
const SUMMARY =
  'Webuddy downloaded the system package. Quit Webuddy before finishing the update from a terminal.'
const COPIED_NOTE =
  `Command copied. Quit Webuddy, run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then reopen Webuddy.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the manual-install copy and recovery actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where Webuddy is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/Copy the command, quit Webuddy/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    const detail = screen.getByText(
      /Webuddy checks the downloaded file against the release metadata/
    )
    ...
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and Webuddy cannot vouch for the file ' +
        'after that point.'
    )
  })
...
describe('LinuxPackageInstallRecoveryCard without a usable command', () => {
  it('promotes Show Package and keeps the official-release fallback', async () => {
    ...
    expect(screen.getByText(/Quit Webuddy before finishing the update/)).toBeTruthy()
