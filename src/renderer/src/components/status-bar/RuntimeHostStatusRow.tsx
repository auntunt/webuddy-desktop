  ...
  switch (state) {
    ...
    case 'runtime-unavailable':
      return translate(
        ...
        'SSH transport is connected, but the Webuddy runtime is unavailable'
      )
    case 'checking':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.checking_host',
        'Webuddy is checking whether this host is reachable'
      )
    case 'reconnecting':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.restoring_connection',
        'Webuddy is trying to restore the connection'
      )
    case 'disconnected':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.host_unreachable',
        'Webuddy isn’t reachable on this host'
      )
  }
}
...
function runtimeFailureExplanation(state: RuntimeHostConnectionState): string | null {
  ...
  if (state === 'runtime-unavailable') {
    return translate(
      ...
      'The remote host may still be running; only the Webuddy runtime connection is unavailable.'
    )
  }
  return translate(
    'auto.components.status.bar.RuntimeHostStatusRow.contact_note',
    'The host may still be running; only the Webuddy connection is unavailable.'
  )
}
