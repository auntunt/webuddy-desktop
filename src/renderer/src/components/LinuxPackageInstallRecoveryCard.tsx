...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Quit Webuddy, run it in a system terminal to install {{value0}}, then reopen Webuddy.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const EXPLAINER = translate(
    ...
    'Copy the command, quit Webuddy, and run it in a system terminal on the computer where Webuddy is installed. Reopen Webuddy after it finishes.'
  )
  ...
  const TRUST_NOTE = translate(
    ...
    'Webuddy checks the downloaded file against the release metadata at the moment it builds this command. The system package itself is not signature-checked, and Webuddy cannot vouch for the file after that point.'
  )
