    ...
    <div className="divide-y divide-border">
      <section className="space-y-2 py-5 first:pt-0">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          {translate(
            ...
            'Shared bundles are not searchable or listed in Webuddy. The link is the credential, so send it only to people you trust.'
          )}
        </p>
      </section>
      ...
      <SettingsSwitchRow
        label={translate(
          ...
          'Allow agents and the Webuddy CLI to publish skill links'
        )}
        description={
          isWebClient
            ? translate(
            ...
            <p className="text-xs leading-relaxed text-muted-foreground">
              {isWebClient
                ? translate(
                    ...
                    'Publishing and link management are available in the Webuddy desktop app.'
                  )
                : translate(
                    'auto.components.settings.shareSkills.signInDescription',
                    'Use your Webuddy account to publish bundles and manage their links. Recipients do not need an account.'
                  )}
            </p>
          </div>
          {!isWebClient ? (
            ...
            >
              {authStatus?.state === 'reconnect-required'
                ...
                : translate('auto.components.settings.shareSkills.signIn', 'Sign in to Webuddy')}
            </Button>
          ) : null}
        </section>
      ) : null}
