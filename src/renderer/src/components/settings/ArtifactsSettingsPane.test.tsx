...
describe('ArtifactsSettingsPane', () => {
  ...
  it('explains the complete sharing workflow', () => {
    ...
    expect(screen.getByText('Manage it in Webuddy')).toBeInTheDocument()
    expect(
    ...
    expect(screen.queryByText('Webuddy account')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Sign in to Webuddy' })).not.toBeInTheDocument()
  })
  ...
  it('loads missing account status and disables sign in until configured', () => {
    ...
    expect(screen.getByRole('button', { name: 'Sign in to Webuddy' })).toBeDisabled()
  })
  ...
  it('points web clients at the desktop app for the opt-in step', () => {
    ...
    expect(
      screen.getByText(/Open Settings → Artifacts in the Webuddy desktop app on the host device/)
    ).toBeInTheDocument()
  })
})
