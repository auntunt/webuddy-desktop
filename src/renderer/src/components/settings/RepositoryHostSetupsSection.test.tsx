...
describe('RepositoryHostSetupsSection', () => {
  it('shows a viewing-host selector when the project has multiple settings-backed hosts', () => {
    ...
    useAppStore.setState({
      ...
      projects: [makeProject({ id: 'github:cloudwave/webuddy' })],
      projectHostSetups: [
        makeSetup({
          id: 'local-repo',
  ...
  it('keeps nested SSH setups distinct and derives readiness from their HUB owner', () => {
    ...
    useAppStore.setState({
      repos: [remoteRepo],
      projects: [makeProject({ id: 'github:cloudwave/webuddy', sourceRepoIds: ['remote-repo'] })],
      projectHostSetups: [
        makeSetup({
          id: 'direct-setup',
          projectId: 'github:cloudwave/webuddy',
  ...
  it('removes independent setup metadata instead of opening an empty repo target', async () => {
    const deleteProjectHostSetup = vi.fn().mockResolvedValue({
      project: makeProject({ id: 'github:cloudwave/webuddy' }),
      setup: makeSetup({
        id: 'gpu-setup',
        projectId: 'github:cloudwave/webuddy',
        repoId: '',
  ...
  it('shows unsupported runtime hosts without enabling setup actions', async () => {
    ...
    expect(container.textContent).toContain('Update Webuddy on this host to set up projects')
