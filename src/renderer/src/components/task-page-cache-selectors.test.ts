...
describe('task page cache selectors', () => {
  ...
  it('scopes repo source rows by source cache scope for retry ownership', () => {
    const localRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:local:github:cloudwave/webuddy'
    }
    const sshRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:ssh:devbox:github:cloudwave/webuddy'
    }

    expect(buildTaskPageRepoSourceState([localRepo, sshRepo], [])).toMatchObject([
      {
        sourceKey: 'repo-1::source:local:github:cloudwave/webuddy'
      },
      {
        sourceKey: 'repo-1::source:ssh:devbox:github:cloudwave/webuddy'
      }
    ])
  })
