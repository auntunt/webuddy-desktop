...
import {
  buildSessionGraph,
  collectGraphPositions,
  reuseUnchangedNodeData,
  toStoredPosition
} from './session-graph-model'
...
// Same cadence as the pop-out relay: status pings burst, the canvas is glanceable.
export const SESSION_CANVAS_STATUS_THROTTLE_MS = 250
...
export function useSessionCanvasData(
  ...
  const agentStatusByPaneKey = useThrottledValue(
    useAppStore((s) => s.agentStatusByPaneKey),
    SESSION_CANVAS_STATUS_THROTTLE_MS
  )
  ...
  const previousGraphRef = useRef<SessionGraph | null>(null)

  const graph = useMemo(
    () =>
      // Why: unchanged cards keep their data object so memoized cards skip unrelated pings.
      reuseUnchangedNodeData(
