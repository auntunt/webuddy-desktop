...
function truncateResult(result: string): string {
  ...
  return chars.length > PASS_ALONG_RESULT_MAX_CHARS
    ? `${chars.slice(0, PASS_ALONG_RESULT_MAX_CHARS).join('')}…`
    : result
}
...
export function composePassAlongPrompt(args: {
  ...
  const result = args.result?.trim() ? truncateResult(args.result) : NO_RESULT_PLACEHOLDER
