      ...
      try {
        ...
        toast.warning(
          translate(
            ...
            'macOS permissions may not reach Webuddy terminals'
          ),
          {
            id: SEVERED_TCC_NOTICE_ID,
            description: translate(
              'auto.hooks.useMacTccAttributionSeveredNotice.description',
              'Running Webuddy terminals are hosted by a daemon started by a previous Webuddy installation. macOS may not apply Webuddy’s Accessibility, Automation, or protected-file permissions to them. Restart the daemon from Manage Sessions to restore access. This will close all running Webuddy terminals.'
            ),
            duration: Infinity,
            action: {
              label: translate(
