...
describe('project-host workspace target resolution', () => {
  ...
  it('chooses the focused host setup when one project exists on multiple hosts', () => {
    ...
    const projects = [makeProject('github:cloudwave/webuddy', ['orca-local', 'orca-ssh'])]
    const projectHostSetups = [
      makeSetup('orca-local', 'github:cloudwave/webuddy', 'local', 'orca-local'),
      makeSetup('orca-ssh', 'github:cloudwave/webuddy', 'ssh:openclaw-2', 'orca-ssh')
    ]
  ...
  it('matches duplicate repo ids to the setup execution host', () => {
    ...
    const projects = [makeProject('github:cloudwave/webuddy', ['orca'])]
    const projectHostSetups = [
      makeSetup('local-setup', 'github:cloudwave/webuddy', 'local', 'orca'),
      makeSetup('ssh-setup', 'github:cloudwave/webuddy', 'ssh:builder', 'orca')
    ]
  ...
  it('resolves an explicit project and host to the matching setup', () => {
    ...
    const projects = [makeProject('github:cloudwave/webuddy', ['orca-local', 'orca-runtime'])]
    const projectHostSetups = [
      makeSetup('orca-local', 'github:cloudwave/webuddy', 'local', 'orca-local'),
      makeSetup('orca-runtime', 'github:cloudwave/webuddy', 'runtime:gpu-1', 'orca-runtime')
    ]
  ...
  it('canonicalizes a stale same-host setup id to the setup the picker shows', () => {
    ...
    const projects = [makeProject('github:cloudwave/webuddy', ['orca-main', 'orca-worktree'])]
    const projectHostSetups = [
      makeSetup('orca-main', 'github:cloudwave/webuddy', 'local', 'orca-main'),
      makeSetup('orca-worktree', 'github:cloudwave/webuddy', 'local', 'orca-worktree')
    ]
  ...
  it('keeps an explicit setup id that is the only one on its host', () => {
    ...
    const projectHostSetups = [
      makeSetup('orca-local', 'github:cloudwave/webuddy', 'local', 'orca-local'),
      makeSetup('orca-ssh', 'github:cloudwave/webuddy', 'ssh:builder', 'orca-ssh')
    ]
  ...
  it('reports unavailable when the project is not set up on the selected host', () => {
    const repo = makeRepo('orca')
    const projects = [makeProject('github:cloudwave/webuddy', ['orca'])]
    const projectHostSetups = [makeSetup('orca', 'github:cloudwave/webuddy', 'local', 'orca')]
  ...
  it('reports setup-not-ready when the selected host has pending setup metadata', () => {
    ...
    const projects = [makeProject('github:cloudwave/webuddy', ['orca'])]
    const projectHostSetups = [
      makeSetup('orca', 'github:cloudwave/webuddy', 'local', 'orca'),
      makeSetup('gpu-pending', 'github:cloudwave/webuddy', 'runtime:gpu', '', {
        path: '',
  ...
  it('reports unavailable when an explicit setup is not ready', () => {
    ...
    const projects = [makeProject('github:cloudwave/webuddy', ['orca'])]
    const projectHostSetups = [
      makeSetup('orca', 'github:cloudwave/webuddy', 'local', 'orca', { setupState: 'setting-up' })
    ]
