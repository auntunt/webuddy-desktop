...
export function activeAgentNotesSendFailureMessage(
  ...
  switch (status) {
    ...
    case 'partial-submit-failed':
      message = options.explicitTarget
        ? 'The notes may already be pasted in the selected terminal, but Webuddy could not submit them.'
        : 'The notes may already be pasted in the active terminal, but Webuddy could not submit them.'
      break
    case 'sent':
      message = ''
      break
