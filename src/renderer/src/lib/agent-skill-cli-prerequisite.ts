...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Webuddy may show a system prompt to register the Webuddy CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Webuddy needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Webuddy CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Webuddy CLI in PATH.'
          )
    )
    return null
  }
  ...
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Webuddy CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Install the Webuddy CLI before running agent skill setup.'
          )
      }
    )
    return
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Webuddy CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'Webuddy could not check your Windows user PATH'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === false) {
    ...
    toast.warning(
      translate(
        ...
        'Webuddy CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Webuddy CLI directory to PATH before setup.'
          )
      }
    )
  }
