...
async function getCreateRepoSelector(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    'Missing repo selector. Pass --repo or run from inside an Webuddy-managed worktree.'
  )
}
  ...
  'worktree rm': async ({ flags, client, cwd, json }) => {
    ...
    if (!hostId) {
      throw new RuntimeClientError(
        ...
        'Webuddy cannot tell which host owns this workspace. Refresh projects and try again.'
      )
    }
