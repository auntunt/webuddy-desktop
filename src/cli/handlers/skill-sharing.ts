...
function requireCloudOperation<T>(operation: SkillCloudOperation<T>): T {
  ...
  if (operation.status === 'reconnect-required') {
    throw new RuntimeClientError('authentication_required', 'Sign in to Webuddy and try again.')
  }
...
async function callShare(
  ...
  try {
    ...
    if (error instanceof RuntimeRpcFailureError && error.code === 'method_not_found') {
      throw new RuntimeClientError(
        'update_required',
        'The connected Webuddy runtime does not support agent skill sharing yet. Update Webuddy on that machine and try again.'
      )
    }
    throw error
  }
