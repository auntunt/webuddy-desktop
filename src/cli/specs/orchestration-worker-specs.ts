...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    summary: 'Start one supervised worker on the Run home or a connected Webuddy server',
    usage:
    ...
    notes: [
      ...
      '--on selects only the worker server; the Run and this command remain on the current Webuddy server.',
  ...
  },
  {
    ...
    notes: [
      ...
      'observation.agentWait names a worker parked on a prompt only a human can answer, with the evidence that proved it (hook, prompt-text, or title). Null means Webuddy looked and found no wait. An absent field means it never looked — an older host, an unverifiable worker identity, an unreadable pane, or an agent probe that did not answer in time — and never means the worker is not waiting. A waiting worker is healthy, not failed.'
    ]
  },
  {
    ...
    notes: [
      ...
      'A returned cursor is pinned to the exact source; start a fresh read if Webuddy reports source_changed.'
    ]
  },
  {
