...
export const PROJECT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['project', 'list'],
    summary: 'List durable projects known to Webuddy',
  ...
  },
  {
    ...
    notes: [
      ...
      '--host runtime:<environment-id> runs the command on that paired Webuddy server instead of filtering this runtime; unknown environment ids are rejected rather than answered with an empty list.',
    ...
    examples: [
      'orca project setups',
      'orca project setups --project github:cloudwave/webuddy',
  ...
  },
  {
    ...
    notes: [
      ...
      '--host runtime:<environment-id> targets that paired Webuddy server; use the id from `orca environment list`, not the environment name.',
      'SSH targets are set up through the desktop UI because the desktop client owns SSH connections.'
    ],
    examples: [
      'orca project setup-existing-folder --project github:cloudwave/webuddy --host local --path ~/orca',
      'orca project setup-existing-folder --project github:cloudwave/webuddy --host runtime:03ef704c-b180-4b10-998d-e28fbd5de9a3 --path /home/me/orca --kind git --json'
    ]
  },
  {
    ...
    examples: [
      'orca project setup-clone --project github:cloudwave/webuddy --host local --url https://github.com/cloudwave/webuddy.git --destination ~/src',
      'orca project setup-clone --project github:cloudwave/webuddy --host runtime:03ef704c-b180-4b10-998d-e28fbd5de9a3 --url https://github.com/cloudwave/webuddy.git --destination /srv --json'
    ]
  },
  {
    ...
    notes: [
      ...
      '--host runtime:<environment-id> targets that paired Webuddy server; use the id from `orca environment list`, not the environment name.',
      'Use setup-existing-folder when Webuddy should import and manage an actual checkout path now.'
    ],
    examples: [
      'orca project setup-create --project github:cloudwave/webuddy --host runtime:03ef704c-b180-4b10-998d-e28fbd5de9a3 --state setting-up --method provisioned --json'
    ]
  },
  {
    ...
    examples: [
      'orca project setup-update --setup github:cloudwave/webuddy::gpu --display-name "GPU VM"',
      'orca project setup-update --setup github:cloudwave/webuddy::gpu --path /srv/orca --state ready --json'
    ]
  },
  {
    ...
    examples: ['orca project setup-delete --setup github:cloudwave/webuddy::gpu --json']
  }
]
