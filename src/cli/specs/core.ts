...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['open'],
    summary: 'Launch Webuddy and wait for the runtime to be reachable',
  ...
  },
  {
    ...
    summary: 'Start Claude Code Agent Teams in the current Webuddy terminal',
    ...
    notes: [
      ...
      'Must be run from inside an Webuddy terminal. Starts Claude Code Agent Teams in the current pane and opens teammates as native Webuddy splits.'
    ],
    examples: ['orca claude-teams', 'orca claude-teams --resume <session-id>']
  },
  {
    path: ['repo', 'list'],
    summary: 'List repos registered in Webuddy',
    usage: 'orca repo list [--json]',
    allowedFlags: [...GLOBAL_FLAGS]
  },
  {
    path: ['repo', 'add'],
    summary: 'Add a project to Webuddy by filesystem path',
  ...
  {
    path: ['worktree', 'list'],
    summary: 'List Webuddy-managed worktrees',
  ...
  },
  {
    ...
    summary: 'Show the Webuddy-managed worktree for the current directory',
    usage: 'orca worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing Webuddy worktree without spelling out $PWD.'
    ],
    examples: ['orca worktree current', 'orca worktree current --json']
  },
  {
    path: ['worktree', 'create'],
    summary: 'Create a new Webuddy-managed worktree',
    usage:
    ...
    notes: [
      ...
      'By default, Webuddy records the new worktree as a child of the caller context when it can infer one from the Webuddy terminal or current directory.',
      'If --repo is omitted, Webuddy infers the repo from the current Webuddy-managed worktree.',
      'Use --project with --host to create on a ready project host setup without spelling the backing repo id.',
      '--host runtime:<environment-id> creates on that paired Webuddy server; use the id from `orca environment list`, not the environment name.',
      ...
      '--no-parent only affects Webuddy lineage; omit --base-branch to use the repo default base, or pass the default base ref explicitly for independent top-level work.',
      'By default this creates the worktree and its first terminal without switching the active Webuddy view.',
    ...
    examples: [
      ...
      'orca worktree create --project github:cloudwave/webuddy --host runtime:03ef704c-b180-4b10-998d-e28fbd5de9a3 --name benchmark --json',
  ...
  {
    path: ['worktree', 'set'],
    summary: 'Update Webuddy metadata for a worktree',
    usage:
  ...
  },
  {
    ...
    destructive: true,
    summary: 'Remove a worktree from Webuddy and git',
    usage:
    ...
    notes: [
      ...
      'For Git worktrees, removal also attempts to delete the checked-out local branch, with or without --force. Webuddy retains branches it knows predated the worktree and any branch whose changes it cannot prove are already merged.'
    ]
  },
  {
    path: ['worktree', 'ps'],
  ...
  {
    path: ['terminal', 'list'],
    summary: 'List live Webuddy-managed terminals',
    usage:
  ...
  },
  {
    ...
    notes: [
      ...
      'A host that cannot apply --shell refuses the create rather than quietly spawning its default shell: macOS and Linux execution hosts spawn the login shell, terminals routed over SSH resolve their shell on the SSH host, a --shell that contradicts the project execution runtime (WSL vs Windows host) is refused, and an Webuddy host older than --shell is refused by the CLI.'
    ],
    examples: [
