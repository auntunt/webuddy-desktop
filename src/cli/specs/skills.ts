...
export const SKILL_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      'The bundle name may be human-readable; Webuddy converts it to a portable lowercase package name.',
  ...
  {
    path: ['skills', 'list'],
    summary: 'List version-matched skill guides bundled with this Webuddy CLI',
    usage: 'orca skills list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Reads bundled guide metadata locally without contacting the Webuddy runtime.',
  ...
  },
  {
    ...
    notes: [
      'Reads bundled guide content locally without contacting the Webuddy runtime.',
  ...
  {
    path: ['skills', 'install'],
    summary: 'Install bundled Webuddy skills via the community skills CLI',
    usage:
      'orca skills install [--skill <name>]... [--all] [--agent <name>[,<name>]] ' +
      '[--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'agent', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the Webuddy runtime.',
      'Resolves to the same `npx skills add <repo> --skill <name> ...` command used by ' +
        'Webuddy Settings, plus the non-interactive flags an unattended host needs ' +
      ...
      'Targets the coding agents Webuddy detects on this host, plus the shared ' +
      ...
      'Use --agent <name>[,<name>...] to choose targets yourself, or --agent universal ' +
        'for the shared directory alone. Required when Webuddy detects no agent.',
  ...
  {
    path: ['skills', 'update'],
    summary: 'Update already-installed Webuddy skills via the community skills CLI',
    usage: 'orca skills update [--skill <name>]... [--all] [--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the Webuddy runtime.',
      'Resolves to the same `npx skills update <names...>` command used by Webuddy Settings, ' +
