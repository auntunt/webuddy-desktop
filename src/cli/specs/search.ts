...
export const SEARCH_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['search'],
    summary: 'Search the full text of agent sessions indexed on the selected Webuddy host',
    usage:
    ...
    notes: [
      'Searches one host: this machine, or the paired Webuddy server named by --environment / --pairing-code. There is no all-computers search.',
      'In an Webuddy SSH terminal, the forwarded CLI searches the controlling Webuddy runtime by default. Use --environment / --pairing-code to select a paired server; --path only filters results on the selected runtime.',
