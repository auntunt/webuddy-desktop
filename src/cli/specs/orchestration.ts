...
export const ORCHESTRATION_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      'To answer a worker question, use orchestration reply --id <msg_id> --body <text> with the same Webuddy CLI executable.',
      ...
      'Use --to dispatch:<id> for attempt-specific coordinator guidance; Webuddy durably relays it to a connected worker server.',
  ...
  },
  {
    ...
    notes: [
      ...
      'completed means the mutation landed and --retry-request replays the recorded outcome instead of starting a second one. pending means the original mutation is still running or Webuddy restarted before recording its outcome; wait for a live original command, otherwise replay with --retry-request.',
